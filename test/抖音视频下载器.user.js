...
(function() {
	...
	var tools = {
		...
		parseData: function(data) {
			...
			if (data.vid) {
				...
				url = video_vid.replace("{vid}", data.vid).replace("{ratio}", ratio === "default" ?
					"1080p" : ratio);
				if (/iesdouyin.com/i.test(location.hostname)) {
		...
		ariaPRC: function(data) {
			...
			$.ajax({
				url: "http://localhost:6800/jsonrpc?tm=1569769711558",
				data: JSON.stringify(d),
				type: "post",
				dataType: "json",
				error: function(xhr) {
					alert(`aria2调用失败!请先打开aria2`);
				},
				success: function(obj) {
					alert(`${d.length}个视频正在通过aria2下载`);
				}
			});
		}
	}
	...
	var set = {
			...
			"video": {
				data: [{
					...
					"value": [{
						"name": "视频地址",
						"key": "default",
						"description": "批量导出模式：仅导出视频地址文本到剪贴板，视频地址间通过换行分割"
					}, {
						"name": "aria2下载",
						"key": "aria2",
						"description": "批量导出模式：通过aria2批量下载当前页已获取到的视频，并根据“视频文件名”选项设置重命名"
					}]
				}, {
					"name": "刷新按钮",
					"type": "text",
					...
					"type": "text",
					"key": "massiveExport",
					"value": "<a style=\"text-decoration:none;\" href=\"javascript:alert('当前页不可用');\">点击批量导出</a>"
				}, {
					"name": "反馈建议",
					"type": "text",
					"key": "feedback",
		...
		extraEventList: {
			export: function() {
				let defaultMode = set.get("exportMode") === "default",
					target;
				if (Page === "user") {
					$(".FeJSrpNN").each((i, e) => {
						e = $(e);
				...
				if (defaultMode) {
					tools.toClipboard(exportData.replace(/\r\n$/, ""), "视频地址已批量导出到剪贴板");
				} else {
					tools.ariaPRC(exportData);
				}
			},
